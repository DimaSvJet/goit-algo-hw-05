...
def benchmark(func: Callable, text_: str, pattern_: str):
    ...
    stnt = f'{func.__name__}(text, pattern)'
    return timeit.timeit(stmt=stnt, setup=setup_code, globals={'text': text_, 'pattern': pattern_}, number = 10)
...
if __name__ == '__main__':
    ...
    false_pattern = "Fake text"
    
    
    results = []
    for pattern in (real_pattern, false_pattern):
    ...
    title = f"{'Алгоритм':<30} | {'Підстрока':<30} | {'Час виконання, сек'}"
    print(title)
    print("-" * len(title))
